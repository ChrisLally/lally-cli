...
import { runGenerateCliCommand } from "./generate/cli/index";
import { runGenerateSdkCommand } from "./generate/sdk/index";

/**
 * @description Initialize, scaffold, validate, clean, and generate Fumadocs documentation assets.
 */
function fumadocsHelp(): string {
  return [
    ...
    "  lally fumadocs generate sdk --app apps/web",
    "  lally fumadocs generate cli --app apps/web",
...
export async function runFumadocsCommand(args: string[]): Promise<number> {
  ...
  try {
    ...
    if (subcommand === "generate") {
      ...
      if (generator === "sdk") {
        return runGenerateSdkCommand(appRoot, generatorArgs);
      }
      if (generator === "cli") {
        return runGenerateCliCommand(appRoot, generatorArgs);
      }
      if (generator !== "sdk" && generator !== "cli") {
        console.error(`Unknown fumadocs generator: ${generator}`);
        console.error("Supported generators: sdk, cli");
        return 1;
      }
    }
  } catch (error) {
